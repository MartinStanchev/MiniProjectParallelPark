...
void setup() {
  ...
  gyro.attach();
  offset = gyro.calibrate();
  Serial.println("offset is " + offset);
}
...
void findSpot(){ //written by majed, martin etc
  //driveForward();
  int spotSize = 0;
  const int ENOUGH_SPACE = 10;
  ...
  while(car.getSpeed()!= 0) {
   ...
   if(rightDistance == 0 || rightDistance > 25){
     ...
     encoderLeft.begin();
      ...
      Serial3.println(" start distance is ");
      Serial3.println(spotStart);
     ...
     spotEnd = encoderLeft.getDistance();
      Serial3.println(" SpotEnd is ");
      Serial3.println(spotEnd);
     
      totalLength =  spotEnd - spotStart; //Spot start is always 0? no good
      if(totalLength > ENOUGH_SPACE) {
       car.stop();
       delay(1000);
       spotSize = totalLength;
       delay(1000);
       Serial3.print(spotSize);
       delay(1000);   
       break;
     }    
  }
